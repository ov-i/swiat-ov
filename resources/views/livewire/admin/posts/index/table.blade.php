<section class="post-table overflow-x-auto py-4" wire:loading.class="opacity-50">
    @if($posts->count())
            ...
            <x-resource-table.body>
                    @foreach ($posts as $post)
                        <x-resource-table.row :resource="$post" :key="$post->getKey()">
                            ...
                            <td class="whitespace-nowrap p-3 text-sm">
                                <div class="inline-flex items-center gap-1 py-1 text-{{ $post->getStatus()->color() }}-600 opacity-75">
